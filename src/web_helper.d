...
static string[ushort] status_code;
static string[string] mimetype_map;
static Sha256 _sha_encoder;


/****m* web_helper/this
 *  FUNCTION
 *    A static constructor that initializes all the globals.
 * SOURCE
 */
public static this() {
 ...
 *  EXAMPLE
 *    string session_id;
 *    session_id = hash_and_base64("1", "secret");
 *    session_id = hash_and_base64("2", "secret");
 *    session_id = hash_and_base64("3", "secret");
 * SOURCE
 */
...
unittest {
	describe("helper#hash_and_base64", 
		it("Should hash and base64 the string", function() {
			string session_id = hash_and_base64("1", "secret");
 ...
 * SOURCE
 */
public static string escape_html(string value) {
 ...
 * SOURCE
 */
public string unescape_html(string value) {
	value = substitute(value, "&amp;", "&");
	value = substitute(value, "&quot;", "\"");
	value = substitute(value, "&gt;", ">");
	value = substitute(value, "&lt;", "<");
...
/****m* web_helper/get_verbose_status_code
 *  FUNCTION
 ...
 * SOURCE
 */
public string get_verbose_status_code(ushort code) {
	return tango.text.convert.Integer.toString(code) ~ " " ~ status_code[code];
}
/*******/

/****m* web_helper/hex_to_char
 *  FUNCTION
 ...
 * SOURCE
 */
private char hex_to_char(char c) {
	if(c >= '0' && c <= '9')
		return c - '0';
	else if(c >= 'A' && c <= 'F')
		return c - 'A' + 10;
 ...
 * SOURCE
 */
private char char_to_hex(char c) {
	char retval;
	if(c >= 0 && c <= 9)
		retval = c + '0';
	else if(c >= 10 && c <= 19)
